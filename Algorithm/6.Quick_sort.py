...
# 使用partition函数，先从右侧开始，right指针指向的数如果小于p，则填入p的空位，left指针指向的数如果大于p，则填入之前right的一个空位

# 时间复杂度：O(nlogn)，其中partition函数的复杂度为O(logn)
# 快排最坏情况：完全逆序的列表，每次只排序一个数，时间复杂度为O(n^2)
# 优化方法：随机选择一个数作为p，只需要将第一个元素与随机选择的数进行对换，其余不变
